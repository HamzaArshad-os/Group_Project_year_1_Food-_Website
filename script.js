...
function ultimatesearch(query){ //self explanatory
    
    let gotquery = String(query).split(',')
    
    let stringedquery = '';
    console.log(gotquery.length)
    for (let i = 0; i < gotquery.length; i++) {
        console.log("processing")
        stringedquery +=gotquery[i] + ','
      }
    var finalstringedquery = stringedquery.substring(0, stringedquery.length - 1);
    console.log(finalstringedquery)
